import { TelegramBot, UpdateType, InlineKeyboardCallbackDataButton } from 'https://deno.land/x/telegram_bot_api/mod.ts'
...
bot.on(UpdateType.CallbackQuery, async ({callback_query}) => {
    ...
    if(allowedUsers.indexOf(userStatus) == -1) {
        bot.answerCallbackQuery({
            ...
            text: `Your status is ${userStatus} but you need to be ${allowedUsers.join(" or ")}`
        })
    } else {
        for(const button of oldInlineKeyboard.flat() as InlineKeyboardCallbackDataButton[]) {
            if(callback_query.data == button.callback_data && button.text.includes(' ')) {
